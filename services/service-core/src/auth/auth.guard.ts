import { Injectable, CanActivate, ExecutionContext } from '@nestjs/common';
import { GqlExecutionContext } from '@nestjs/graphql';
import * as jwt from 'jsonwebtoken';

/*Receive, validate, decode token, 
  return true if header exists and token is valid
  return false if no header or expired, invalid token
*/

@Injectable()
export class KeycloakAuthGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean | Promise<boolean> {
    const ctx = GqlExecutionContext.create(context);
    const { req } = ctx.getContext();

    const authHeader = req.headers.authorization;

    //public key from keycloak realm
    const public_key =
      '';
    const key = `-----BEGIN PUBLIC KEY-----\n${public_key}\n-----END PUBLIC KEY-----`;

    if (authHeader) {
      const token = authHeader.split(' ')[1];
      try {
        const decoded = jwt.verify(token, key, { algorithms: ['RS256'] });
        console.log(decoded);
        return true;
      } catch (error) {
        return false;
      }
    }
    return false;
  }
}
